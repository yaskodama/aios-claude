...
さらに \code{.aipl} (AIPL 実行プログラム) を出力し,アクター並列実行系
...
\begin{tikzpicture}[node distance=4mm and 7mm, every node/.style={font=\small}]
  ...
  \node[draw, rounded corners, right=of ga, fill=green!10] (abcl) {\code{problem.aipl}};
...
$\to$ \code{.aipl} の三段パイプライン,\S\ref{sec:examples} で具体例,
...
\begin{description}[leftmargin=*]
  ...
  \item[Stage 3.] \textbf{\code{.aipl} への codegen}: GA 評価ループ全体
    を AIPL のアクター並列プログラムとして合成.Phase 5 で実装.
\end{description}

生成された \code{.aipl} を AIPL ランタイム (Python / OCaml / 各種 codegen)
...
\subsection{\code{.aipl} への codegen: AIPL オーケストレータ生成}
