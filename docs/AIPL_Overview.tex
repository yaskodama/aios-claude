...
\midrule
11a--e & typed annotations / unions / generics / length-arrays & \texttt{Typecheck*.abcl} \\
12 & capability effects \texttt{!\{fs,ai,net,mut\}} & \texttt{Effects.abcl} \\
13 & CSP channels & \texttt{Channels.abcl} \\
14 & linear / use-after-move & \texttt{Linear.abcl} \\
15 & owned (\texttt{pub} field visibility) & \texttt{Owned.abcl} \\
16 & transient cast at any-boundary & \texttt{Transient.abcl} \\
17 & structured concurrency (\texttt{scope \{ future ... \}}) & \texttt{Phase17\_StructuredConc.abcl} \\
\bottomrule
\end{tabular}
\end{center}
...
各 Level B-N の検査器は \texttt{SampleSelfConsistency.abcl} で
...
\item Python runtime: 33/33 \texttt{.abcl} + 21/21 \texttt{.aipl} (Phase A-G)
