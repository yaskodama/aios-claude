...
第 \ref{sec:dcache} 節に示す。さらに Raspberry Pi 4（Cortex-A72）・5（Cortex-A76）でも同一のハーネスで測定し、
「計算はスケールし、メモリはスケールしない」という結論が A53・A72・A76 の
3 世代を通じて一般化することを確認した（第 \ref{sec:cross} 節）。
\end{abstract}
...
\begin{table}[htbp]
\centering
\caption{ボード別 ワーカープール型 SMP の比較（A53/A72/A76 3 世代）}
\label{tab:cross}
\small
...
計算 \bench{dining} & \spd{3.99}  & \spd{3.99} & \spd{3.99} & 計算律速（公平比較） \\
メモリ \bench{fill} 1\,MB & \spd{0.9}--\spd{1.35} & \spd{1.38} & \spd{1.06} & メモリ律速（公平比較） \\
プール往復 & 7--11\,\textmu s      & 4\,\textmu s & 3\,\textmu s & 並列化の損益分岐 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{横断表の実測検証（反復測定）}
\label{sec:cross-verify}

表 \ref{tab:cross} の Pi 4／Pi 5 の値は、単発ではなく\textbf{稼働中の実機で反復測定して
確認}した（\route{/bench}、2026 年 7 月、Pi 4 = \texttt{192.168.3.100}、
Pi 5 = \texttt{192.168.3.101}）。

\begin{itemize}[leftmargin=1.6em,noitemsep]
  \item \textbf{\bench{dining} \spd{3.99} は本物}。5 回測定で Pi 4・Pi 5 とも
        \texttt{x100} の最小＝中央＝最大が \texttt{399}（完全に安定）。3 世代の
        \spd{3.99} は偶然の一致ではなく、各ボードで独立に得た実測である。
  \item \textbf{\bench{fill} 1\,MB}：Pi 4 は 8 回で \spd{1.32}--\spd{1.41}（中央
        \spd{1.37}）、Pi 5 は \spd{0.99}--\spd{1.34}（中央 \spd{1.06}）。Pi 5 の
        ばらつきが大きいのは約 8 秒周期の自動アクタ GC の混入で、これも\textbf{単発
        測定を信じてはいけない}ことの実例である（第 \ref{sec:clock} 項の時計問題と
        同じ教訓）。
  \item \textbf{プール往復}：Pi 4 は空ジョブ 4 コア投入を 15 回測って \texttt{4--5}\,
        \textmu s（実質 4\,\textmu s）、Pi 5 は最小ジョブ（\bench{fill} 1 語）の 4 コア
        投入で 2\,\textmu s であり、いずれも表の値と整合する。
  \item \textbf{\bench{primes}}（連続分割）は Pi 4 \spd{3.00}・Pi 5 \spd{3.02} で
        安定（各 5 回で不動）。ただし分割方式に依存するため横断の主表には載せず、
        第 \ref{sec:cross-fair} 項で別途扱う。
\end{itemize}

\subsection{公平な比較の条件：同一の分割方式のみ}
\label{sec:cross-fair}

Pi 4（A72）・Pi 5（A76）でも同一の \route{/bench} ハーネス（同一ワークロード・
\texttt{agree} 検証・\textmu s タイミング・\texttt{cores} スイープ）で測定した。
ただし\textbf{全ベンチが素朴には比較可能でない}。Pi 3 と Pi 4/Pi 5 は N-Queens と
素数の\textbf{仕事の配り方が異なっていた}——Pi 3 はストライド／least-loaded で配り、
Pi 4 は既存の連続分割である。そこで本セッションで、\textbf{配り方を揃える実験}を
Pi 4 実機で行い、差が「分割方式」で説明できるかを直接検証した。

\subsubsection*{実験：N-Queens は静的分割では揃わない（連続でもインターリーブでも）}

Pi 4 の \route{/bench} に\textbf{インターリーブ分割}（\texttt{?il=1}：第 1 行カラムの
剰余類）と、Pi 3 と同型の\textbf{第 2 行細粒度インターリーブ}（\texttt{?il=2}：合法な
(row0,\,row1) 対の剰余類、$\sim n(n{-}3)$ 項）を実装し、同一クロックで測り直した
（全て \texttt{agree=yes}）。

\begin{table}[htbp]
\centering
\caption{Pi 4（A72）N-Queens $n{=}12$：静的分割の三方式（4 コア速度向上）}
\label{tab:pi4-nq-split}
\small
\begin{tabular}{@{}lccl@{}}
\toprule
分割方式 & 4 コア速度向上 & コア数スイープ(1/2/3/4) & \\
\midrule
連続（既存）                 & \spd{2.04} & \spd{1.00}/\spd{1.60}/\spd{1.79}/\spd{1.85} & \\
第 1 行インターリーブ        & \spd{1.83} & --- & \\
第 2 行インターリーブ（Pi 3 と同型） & \spd{1.99} & \spd{1.00}/\spd{1.60}/\spd{1.77}/\spd{1.83} & ← 最細の静的分割 \\
\bottomrule
\end{tabular}
\end{table}

\noindent
表 \ref{tab:pi4-nq-split} の結論は明快である：\textbf{分割方式を変えても Pi 4 の
N-Queens は \spd{2.0} 前後で頭打ち}になる。しかもこれは規模不変で、$n{=}12/13/14$
のいずれでも第 2 行インターリーブは \spd{1.99}/\spd{2.01}/\spd{2.04} であった
（$n$ を上げても改善しない）。コア数スイープが \spd{1.60}$\to$\spd{1.77}$\to$\spd{1.83}
と逓減して平坦化するのは、\textbf{少数の重い部分木が支配し、行単位の静的分割では
それ以上割れない}——典型的な負荷不均衡である。

したがって、\bench{nqueens} の Pi 3 \spd{3.65} 対 Pi 4 \spd{2.05} という差は
\textbf{「連続 vs インターリーブ」では説明できない}（連続もインターリーブも第 2 行
細分も、Pi 4 では等しく \spd{2.0}）。差の真因は\textbf{静的分割 vs 動的ロード
バランス}である：Pi 3 の高い値は least-loaded に適応配分する動的ディスパッチャ
（\route{/api/loadbal/nqueens}）由来であり、これは静的分割（本レポートの \route{/bench}）
とは別階層（Level 3＝動的ワークスティーリング）の機構である。N-Queens は、
コスト分布が予測不能かつ極端に偏るため、\textbf{静的にはどう配っても揃わず、
動的負荷分散を要する}——これが実測から得た精緻化である。

\subsubsection*{一方 素数は、分割方式を揃えれば公平に比較できる}

対照的に \bench{primes} は、Pi 4 をインターリーブ（\texttt{?il=1}）にすると
\textbf{Pi 3 の偶奇共鳴を再現}した——2 コアで \spd{1.01}（コア 0 が全偶数を担当し
即座に非素数と判定して遊ぶ）、4 コアで \spd{2.02}。連続分割の Pi 4 \spd{3.00} とは
別物だが、\textbf{Pi 3 と同じストライドにすれば同じ \spd{2.0} 級の共鳴パターンに
一致}する。すなわち素数は「分割方式を揃えれば公平に比較できる」ベンチである。

\medskip
\noindent
まとめると、公平に横断比較できるのは、\textbf{均一コストゆえ静的分割で揃う
\bench{dining}・\bench{fill}}（両ボード連続）と、\textbf{分割方式を明示的に揃えた
\bench{primes}}（両ボード・ストライド）である。\bench{nqueens} だけは静的分割の
射程外（動的負荷分散が要る）であり、静的ハーネスの範囲では公平な数値比較の対象と
しない。

\subsection{知見は微アーキを超えて一般化する}

その 2 つで、本レポートの中心的結論が A53 から A72・A76 へ\textbf{一般化する}ことが
確認できた（3 世代のマイクロアーキテクチャすべて）。

\begin{itemize}[leftmargin=1.6em]
  \item \textbf{計算律速は 3 世代すべてで理想的にスケールする。}\bench{dining} の
        コア数スイープは A53・A72・A76 とも
        \spd{1.00}/\spd{1.99}/\spd{2.99}/\spd{3.99} と\textbf{同一の完全な直線}を
        描く。ワーカープール型 SMP による計算の線形スケーリングは、
        マイクロアーキテクチャに依存しない。
  \item \textbf{メモリ律速は 3 世代すべてでスケールしない。}\bench{fill} 1\,MB は
        A53 で \spd{0.9}--\spd{1.35}、A72 で \spd{1.40}、A76 で \spd{1.02} と、
        いずれも\textbf{平坦}で線形から遠い。D-cache OFF ゆえメモリ書込が
        RAM 帯域律速になる現象は、世代を問わず共通である。A76 は単一コアの
        絶対速度が最も速い（\bench{fill} 1\,MB の 1 コアが約 2100\,\textmu s で、
        A72 の約 7000\,\textmu s より速い）が、それでも 4 コアでスケールしない。
\end{itemize}

\noindent
すなわち「計算はスケールし、描画（メモリ）はスケールしない」という Pi 3 の
結論は、Pi 4・Pi 5 でもそのまま成り立つ。異なるのは絶対速度と細部だけである。
\textbf{Raspberry Pi 3（A53）・4（A72）・5（A76）の 3 世代を通じて、
並列化すべき対象を分ける境界は同一}であった。

% =======================================================================
\section{設計空間 GA：予測 vs 実機実測}
\label{sec:ga}
\input{experiments/ga}
