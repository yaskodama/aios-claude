document.addEventListener("DOMContentLoaded", () => {
  ...
  async function poll() {
    try {
      // logs (print)
      const r1 = await fetch('/api/log?after=' + afterLog);
      if (r1.ok) {
        const j1 = await r1.json();
        if (typeof j1.next === 'number') afterLog = j1.next;
        if (j1.lines) {
      ...
      // events (reply)
      const r2 = await fetch('/api/events?after=' + afterEvt);
      if (r2.ok) {
        const j2 = await r2.json();
        if (typeof j2.next === 'number') afterEvt = j2.next;
        if (j2.lines) {
...
<body>
  ...
  <div id="inputRow">
   ...
   consoleOut.textContent =
      ...
      "If /api/repl is not implemented yet, commands will not execute.\\n\\n";
  }
  ...
  function installButtons() {
    ...
    newBtn.textContent = "New Actor (via REPL)";
    newBtn.style.marginRight = "8px";
    newBtn.onclick = () => {
      if (out) out.textContent = "Use Open Console to run: var c = new Calc();";
    };
