(* avm_gen.ml — compile AIPL classes to a ".avm" actor-bytecode module that the
   ...
   Supported subset (the kernel VM is integer-only):
     - integer fields (`var x = 0;` in a class), method int params
     - arithmetic + comparison: + - * / %  <  <= > >= == !=
     - control flow: if / while
     - assignment to a field
     - send a.m(args)   (async; a = self or a field holding an actor id)
     - new C(...)       -> spawn an actor of class C (ctor args ignored)
     - print(e)
   Unsupported constructs (now/future/await/select/saga/strings/floats/records,
   method-local vars, return value) raise a clear error.
...
let op_print=0x42 and op_ret=0x43
...
(* module-global string pool, in insertion order *)
let strs : (string, int) Hashtbl.t = Hashtbl.create 32
let str_rev = ref []           (* strings in REVERSE insertion order *)
let str_n = ref 0
let sid s =
  match Hashtbl.find_opt strs s with
  | Some i -> i
  | None -> let i = !str_n in Hashtbl.replace strs s i; str_rev := s :: !str_rev; incr str_n; i

(* class name -> index in the module (for SPAWN of `new C(...)`) *)
...
(* compile one method body to bytecode bytes *)
let compile_method ~fields ~params (body : stmt) : string =
  ...
  let fixups = ref [] in                         (* (pos, label) — u16 to backpatch *)
  ...
  let resolve n =                                 (* a name -> load it onto the stack *)
  ...
  let rec ce (e : expr) =
    ...
    | Call ("print", [a]) -> ce a; u8 op_print
    | _ -> failwith "avm: unsupported expression (only int / var / arithmetic / new / print)"
  in
  ...
  let rec cs (s : stmt) =
    ...
    | Assign (n, e) ->
        ...
        (match idx_of n fields with
         ...
         | None -> failwith ("avm: assignment to non-field '" ^ n ^ "' (method-local vars unsupported)"))
    | If (c, t, f) ->
    ...
    | UnsafeSend (tgt, m, args) ->
        resolve (target_name tgt); List.iter ce args;
        u8 op_send; u16 (sid m); u8 (List.length args)
    | CallStmt ("print", [a]) -> ce a; u8 op_print
    | CallStmt (f, _) -> failwith ("avm: unsupported call '" ^ f ^ "' (only print)")
    | VarDecl _ | TypedVarDecl _ -> failwith "avm: method-local var unsupported (use class fields)"
    | Return _ -> ()                              (* no reply path in the VM — ignore *)
    | _ -> failwith "avm: unsupported statement (now/future/await/select/saga/scope/become)"
  in
  cs body; u8 op_ret;
...
let gen_program_avm (prog : program) : string =
  ...
  let classes = List.filter_map (function Class c -> Some c | _ -> None) prog in
  List.iteri (fun i c -> Hashtbl.replace class_index c.cname i) classes;
  (* compile every method first so all SEND method-name strings get pooled *)
  let compiled =
    List.map (fun c ->
