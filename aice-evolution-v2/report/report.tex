...
(Actor-based Intelligent Parallel Language; 旧称 AIPL) アクター並列
...
\texttt{examples/*.aice} (5 ファイル) & $\rightarrow$ & \texttt{out/*.ga.json} \\
                                      & $\rightarrow$ & \texttt{out/*.aipl} (\,オプション\,) \\
...
Python パイプラインと並行して、生成された 5 つの \texttt{.aipl} を
...
\begin{table}[h]
\centering
\caption{AIPL ランタイム実行結果 (\texttt{.aipl})}
...
\begin{itemize}
  ...
  \item \texttt{out/*.aipl} (各 \(\sim\)19 KB)
