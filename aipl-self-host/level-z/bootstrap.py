...
# Reads a user .aipl, concats the AIPL-side lexer + parser + eval + driver,
# then hands the lot to the host AIPL runtime.  No analysis on the host side.
import os, pathlib, runpy, sys
HERE = pathlib.Path(__file__).resolve().parent
ROOT = HERE.parents[1]                                  # repo root
LC   = ROOT / "aipl-self-host" / "level-c"
USER = pathlib.Path(sys.argv[1]).resolve()              # user .aipl
TMP  = pathlib.Path("/tmp/_aipl_level_z.aipl")
parts = [(LC / f).read_text() for f in ("lexer.aipl", "parser.aipl", "eval.aipl")]
parts.append((HERE / "driver_head.aipl").read_text())
