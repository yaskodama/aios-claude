...
\texttt{abclc/TypedDemo.aipl} で同じ事象}が起きていることに気付いた:

\begin{lstlisting}[language=,caption={TypedDemo.aipl ですら type-check に失敗}]
$ ./_build/default/src/aipl2c.exe abclc/TypedDemo.aipl --check
class Counter
  bump : (int) -> int
[Type error] line 37, col 13: unbound variable: count
\end{lstlisting}

\texttt{TypedDemo.aipl} は\texttt{ var count: int = 0;} という\textbf{
...
追加チェックする.これで\texttt{ TypedDemo.aipl} を始めとした既存の
...
\subsection{Rotate4LinesXinu.aipl}
...
\begin{lstlisting}[language=abcl,caption={Rotate4LinesXinu.aipl 主な差分}]
class Spinner {
  var idx: int = 0;
  var angle: float = 0.;
  var speed: int = 5;
...
\subsection{Philosophers5Xinu.aipl}
...
\begin{lstlisting}[language=abcl,caption={Philosophers5Xinu.aipl actor 参照フィールド}]
class Philosopher {
  var idx: int       = 0;
  var state: int     = 0;
  var counter: int   = 0;
...
\subsection{BoundedBufferXinu.aipl}
...
\begin{lstlisting}[language=abcl,caption={BoundedBufferXinu.aipl Producer のフィールド}]
class Producer {
  var idx: int     = 0;
  var state: int   = 0;
  var counter: int = 0;
...
TypedDemo.aipl} 自体が R0 修正によって\textbf{ 初めて型検査を
通るようになった}.
